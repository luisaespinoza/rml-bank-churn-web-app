...
export function normalizeRawInput(rawInput) {
  return {
    CreditScore: numberOrDefault(rawInput.CreditScore, 650),
    Geography: String(rawInput.Geography ?? "France"),
    Gender: String(rawInput.Gender ?? "Female"),
    Age: numberOrDefault(rawInput.Age, 40),
    Tenure: numberOrDefault(rawInput.Tenure, 5),
    Balance: numberOrDefault(rawInput.Balance, 0),
    NumOfProducts: numberOrDefault(rawInput.NumOfProducts, 1),
    HasCrCard: numberOrDefault(rawInput.HasCrCard, 1),
    IsActiveMember: numberOrDefault(rawInput.IsActiveMember, 1),
    EstimatedSalary: numberOrDefault(rawInput.EstimatedSalary, 100000)
  };
}
